...
| 键 | 类型/默认 | 生效方式 |
|----|-----------|----------|
| `readingMode` | `'horizontal'`(默认)/`'verticalPaged'`/`'verticalScroll'` | **当前状态**(W19 修订):进书页为竖排值 → 自动恢复竖排+进度;退出竖排写回 `'horizontal'` |
| `verticalLastMode` | `'verticalPaged'`(默认)/`'verticalScroll'` | 上次竖排预设偏好(W19):退出后再进竖排按它还原 |
...
- **宽度封顶必须连带收窄滚动容器**(W19):只封顶排版宽而容器占满视口时,邻页列会从 scroll-padding 留白里漏出来,用户看到"翻半页"。页窗(容器)=封顶宽居中,所见即一页。
- **Header 栈里的书页按钮要等 book 就绪**(W19):Header 按路径判断早于数据到达,入口过早可点会使依赖 DOM 锚元素的逻辑(firstVisibleBlock)静默失效;条件必须含 `book &&`。
- **book 页是 body 滚动**(W19 排查副产物):站点全局 CSS 使 body 成为滚动容器(documentElement.scrollHeight≈视口),window.scrollY 恒 0;滚动断言/操作以元素 rect 为准,勿读 window.scrollY。
